// import { isProbablyPrime } from "./potentially-prime";
...
export class PrimeCounter {
    ...
    private isPrime(potentiallyPrime: number) {
        for (let i = 5; i * i <= potentiallyPrime; i += 6) {
            if (potentiallyPrime % i === 0 || potentiallyPrime % (i + 2) === 0) {
              return false;
            }
          }
        return true;
    ...
    countPrimes(numbers: number[]) {
        for (const number of numbers) {
            if(!isProbablyPrime(number)) continue;
